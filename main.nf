...
========================================================================================
                         nf-core/enaprep
========================================================================================
 nf-core/enaprep Analysis Pipeline.
...
def helpMessage() {
    ...
    Mandatory arguments:
      --reads                       Path to input data (must be surrounded with quotes). Default = ${params.reads}
    ...
    Other options:
      ...
      --email                       Set this parameter to your e-mail address to get a summary e-mail with details of the run sent to you when the workflow exits. Default = ${params.email}
      --email_on_fail               Same as --email, except only send mail if the workflow is not successful. Default = ${params.email_on_fail}
      --maxMultiqcEmailFileSize     Theshold size for MultiQC report to be attached in notification email. If file generated by pipeline exceeds the threshold, it will not be attached (Default: ${params.maxMultiqcEmailFileSize})
...
def create_workflow_summary(summary) {
    ...
    yaml_file.text  = """
    id: 'nf-core-enaprep-summary'
    description: " - this information is collected when the pipeline is started."
    section_name: 'nf-core/enaprep Workflow Summary'
...
workflow.onComplete {
    ...
    def subject = "[nf-core/enaprep] Successful: $workflow.runName"
    if (!workflow.success) {
      subject = "[nf-core/enaprep] FAILED: $workflow.runName"
    }
    def email_fields = [:]
    ...
    try {
        if (workflow.success) {
            ...
            if (mqc_report.getClass() == ArrayList) {
                log.warn "[nf-core/enaprep] Found multiple reports from process 'multiqc', will use only one"
                mqc_report = mqc_report[0]
            }
        }
    } catch (all) {
        log.warn "[nf-core/enaprep] Could not attach MultiQC report to summary email"
    }
    ...
    if (email_address) {
        try {
          ...
          log.info "[nf-core/enaprep] Sent summary e-mail to $email_address (sendmail)"
        } catch (all) {
          // Catch failures and try with plaintext
          [ 'mail', '-s', subject, email_address ].execute() << email_txt
          log.info "[nf-core/enaprep] Sent summary e-mail to $email_address (mail)"
        }
    }
    ...
    if (workflow.success) {
        log.info "${c_purple}[nf-core/enaprep]${c_green} Pipeline completed successfully${c_reset}"
    } else {
        checkHostname()
        log.info "${c_purple}[nf-core/enaprep]${c_red} Pipeline completed with errors${c_reset}"
    }
...
def nfcoreHeader(){
    ...
    ${c_blue}  | \\| |       \\__, \\__/ |  \\ |___     ${c_green}\\`-._,-`-,${c_reset}
                                            ${c_green}`._,._,\'${c_reset}
    ${c_purple}  nf-core/enaprep v${workflow.manifest.version}${c_reset}
    -${c_dim}--------------------------------------------------${c_reset}-
    """.stripIndent()
}
